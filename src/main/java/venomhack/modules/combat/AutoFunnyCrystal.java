...
public class AutoFunnyCrystal extends ModuleHelper implements IModule {
                ...
                if (!errorMsg.isEmpty()) {
                    ...
                    if (this.trap.get() && (mItem.equals(Items.OBSIDIAN) || this.switchDelayLeft <= 0)) {
                        for (Vec3i offset : CityUtils.CITY_WITHOUT_BURROW) {
                            ...
                            if (this.mc.world.getBlockState(trapPos).getMaterial().isReplaceable() && this.mc.world.canPlace(Blocks.OBSIDIAN.getDefaultState(), trapPos, ShapeContext.absent())) {
    ...
    private void add(List<BlockPos> list, BlockPos pos) {
        if (!list.contains(pos) && this.mc.world.getBlockState(pos).getMaterial().isReplaceable()) {
            if (this.airPlace.get()) {
                list.add(pos);
    ...
    private boolean findNeighbour(List<BlockPos> list, BlockPos pos, int iteration) {
        if (iteration > 3) {
            ...
            for (Direction direction : Direction.values()) {
                if (this.mc.world.canPlace(placeState, pos.offset(direction), ShapeContext.absent()) && (list.contains(pos.offset(direction)) || !this.mc.world.getBlockState(pos.offset(direction)).getMaterial().isReplaceable())) {
                    return true;
                }
            }
    ...
    private void badTarget(PlayerEntity target, String message) {
        if (this.noEntities(target)) {
            ...
            this.info("Funny crystalling " + this.target.getEntityName() + "...");
        }
    }
