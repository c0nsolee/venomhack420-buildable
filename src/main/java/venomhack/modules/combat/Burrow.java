...
public class Burrow extends ModuleHelper {
                ...
                if (BlockUtils2.invalidPos(playerBlock)) {
                    ...
                    if (hitResult == null) {
                        ...
                        if (!this.phoenixMode.get() && this.collides(Math.ceil(eyeHeight))) {
                            ...
                            if (!state.getMaterial().isReplaceable() && (!(state.getBlock() instanceof BedBlock) || this.mc.world.getRegistryKey() == World.OVERWORLD) && state.getBlock().getHardness() > 0.0F && this.collides(onEchest ? 1.0 : 0.0)) {
    ...
    private class StaticListener {
        ...
        private void surroundListener(Post event) {
            --this.delay;
            if (UtilsPlus.isObbyBurrowed(Burrow.this.mc.player) && Burrow.this.mc.world.getBlockState(Burrow.this.mc.player.getBlockPos()).getBlock().getBlastResistance() > 600.0F) {
                this.pos = Burrow.this.mc.player.getBlockPos();
            } else if (!Burrow.this.isActive() && Burrow.this.mc.player.isOnGround() && !UtilsPlus.isBurrowed(Burrow.this.mc.player) && (!Burrow.this.pauseEating.get() || !Burrow.this.mc.player.isUsingItem() || !Burrow.this.mc.player.getMainHandStack().isFood() && !Burrow.this.mc.player.getOffHandStack().isFood())) {
                    ...
                    if (this.delay <= 0 && Burrow.this.autoTrap.get() && UtilsPlus.isSurrounded(Burrow.this.mc.player, false, false)) {
                        for (PlayerEntity enemy : Burrow.this.mc.world.getPlayers()) {
                            if (!Burrow.this.mc.player.equals(enemy) && !(Burrow.this.mc.player.distanceTo(enemy) > 5.0F)) {
                                ...
                                if (Friends.get().shouldAttack(enemy) && !UtilsPlus.isSurrounded(enemy, true, true) && Burrow.this.mc.player.getPos().add(0.0, 1.0, 0.0).distanceTo(enemy.getPos()) <= 2.0 && enemy.getY() > Burrow.this.mc.player.getY()) {
                                    Burrow.this.toggle();
                                    this.delay = 20;
                                    return;
                                }
