...
public class Surround extends ModuleHelper {
                    ...
                    try {
                        for (Entity entity : this.mc.world.getOtherEntities(null, placeShape.isEmpty() ? new Box(pos) : placeShape.getBoundingBox().offset(pos), Entity::canHit)) {
                            if (entity instanceof EndCrystalEntity) {
                                ...
                                if (this.attackCrystals.get() && this.attackDelayLeft <= 0 && this.swapPenaltyTimer.passedMillis(this.attackSwapPenalty.get().intValue()) && (double) this.mc.player.distanceTo(entity) <= this.attackRange.get() && entity.age >= this.attackMinAge.get()) {
                                    crystal = entity;
                                    continue;
                                }
                            }
    ...
    private boolean findNeighbour(Set<BlockPos> set, BlockPos pos, int iteration, BlockState placeState, int playerEyeY) {
        for (Direction direction : Direction.values()) {
            ...
            if ((!this.strictDirections.get() || AntiCheatHelper.isInteractableStrict(this.mc.player.getBlockX(), playerEyeY, this.mc.player.getBlockZ(), neighbour, direction.getOpposite())) && (set.contains(neighbour) || Statistics.get().pendingObsidian.containsKey(neighbour) || !this.mc.world.getBlockState(neighbour).getMaterial().isReplaceable())) {
                set.add(pos);
                return true;
            }
        }
    ...
    private class StaticListener {
        ...
        private void surroundListener(meteordevelopment.meteorclient.events.entity.player.SendMovementPacketsEvent.Post event) {
            if (Surround.this.auto.get() && !Modules.get().isActive(Surround.class) && (!Surround.this.onlyGround.get() || Surround.this.mc.player.isOnGround()) && UtilsPlus.obbySurrounded(Surround.this.mc.player)) {
