...
public class PistonAura extends ModuleHelper {
    ...
    private final Setting<DamageCalc> damageCalc = this.setting("damage-calc-position", "Where to check for crystal damage", DamageCalc.PLACEPOS);
    ...
    private final Setting<Integer> blockingBreakDelay = this.setting("blocking-break-delay", "Time in ticks for when to break a blocking crystal", Integer.valueOf(5), 0.0, 10.0);
    private final Setting<Boolean> swing = this.setting("swing", "Swing hand", Boolean.valueOf(true), this.sgToggles);
    private final Setting<Boolean> disableWhenNone = this.setting("disable-when-none", "Disables the module when out of resources", Boolean.valueOf(true), this.sgToggles);
    private final Setting<Boolean> strict = this.setting("strict", "Strict mode", Boolean.valueOf(false), this.sgToggles);
    private final Setting<Boolean> antiSuicide = this.setting("anti-suicide", "Prevents you from dying (doesnt seem to work)", Boolean.valueOf(false), this.sgToggles);
    ...
    private final Setting<Boolean> torchSupport = this.setting("torch-support", "Whether to place support blocks for redstone torches", Boolean.valueOf(true), this.sgToggles);
    private final Setting<Boolean> crystalSupport = this.setting("crystal-support", "whether to place support blocks for end crystals", Boolean.valueOf(true), this.sgToggles);
    private final Setting<Boolean> debugRender = this.setting("render-place-positions", "whether to render placement positions", Boolean.valueOf(true), this.sgToggles);
    ...
    public void onTickPost(Post event) {
        if (this.postAction != null && !this.mining) {
            if (this.stage != Stage.SEARCHING || this.pistonPos == null || this.faceOffset == null) {
                this.tickCounter = 0;
                this.postAction.run();
                ...
                Rotations.rotate(yaw, 0.0, () -> this.hasRotated = true);
            } else {
                this.tickCounter = 0;
                this.postAction.run();
                this.postAction = null;
        ...
        } else if (this.torchPos != null && this.mc.world.getBlockState(this.torchPos).isAir()) {
            this.mining = false;
        }
    }
    ...
    public void onReceivePacket(Receive event) {
        ...
        if (deadEntity instanceof BlockUpdateS2CPacket packet && this.torchPos != null && packet.getPos().equals(this.torchPos) && packet.getState().isAir()) {
            this.miningTicks = 0;
        ...
        if (!this.strict.get() || !(this.mc.player.getVelocity().length() > 0.08)) {
            switch (this.stage) {
                ...
                case REDSTONE -> {
                    ...
                    if (this.canCrystalSupport && this.getBlock(this.crystalPos.down()) != Blocks.OBSIDIAN || this.getCrystalAtPos(this.crystalPos) == null) {
                        this.stage = Stage.CRYSTAL;
                        return;
                    }
                ...
                case BREAKING -> {
                    ...
                    if (this.crystalPos != null) {
                        ...
                        double damage = this.damageCalc.get() == DamageCalc.PLACEPOS ? DamageUtils.crystalDamage(this.target, new Vec3d((double) this.facePos.getX() + 0.5, (double) this.facePos.getY() + offsetForBlastResistant, (double) this.facePos.getZ() + 0.5), false, null, blastResistantAtFace) : DamageUtils.crystalDamage(this.target, this.crystal.getPos().add(0.0, blastResistantAtFace ? -0.5 : 0.0, 0.0), false, null, blastResistantAtFace);
                        if (this.debugPrint.get()) {
                    ...
                    } else if (this.pistonPos != null && this.pistonHeadBlocking(this.pistonPos) || this.crystalPos != null && !this.mc.world.getBlockState(this.crystalPos).getMaterial().isReplaceable()) {
                        this.postAction = () -> {
                            if (this.mine.get() && this.torchPos != null && this.torchPos.equals(this.pistonPos.down())) {
                                this.mine(this.torchPos, false);
                            }
    ...
    private boolean pistonHeadBlocking(BlockPos pos) {
        for (Direction direction : new Direction[]{Direction.SOUTH, Direction.WEST, Direction.EAST, Direction.NORTH}) {
    ...
    private boolean evaluateTarget(PlayerEntity candidate) {
        BlockPos tempFacePos = new Mutable(candidate.getPos().x, candidate.getPos().y, candidate.getPos().z).up();
        if (this.evaluateTarget(tempFacePos, candidate)) {
            return true;
        } else {
            return this.evaluateTarget(tempFacePos.up(), candidate) || this.evaluateTarget(tempFacePos.up(2), candidate);
        }
    }
    ...
    private boolean evaluateTarget(BlockPos tempFacePos, PlayerEntity candidate) {
        ...
        if (blockingCrystal != null) {
            if (this.debugPrint.get()) {
                this.info("breaking due to delay");
            }
            ...
            for (Direction faceOffset : new Direction[]{Direction.SOUTH, Direction.WEST, Direction.EAST, Direction.NORTH}) {
                ...
                if (this.canPlaceCrystal(potentialCrystal.down(), this.canCrystalSupport)) {
                    ...
                    double offsetForBlastResistant = blastResistantAtFace ? 0.0 : 0.5;
                    Vec3d calculatedCrystalPos = new Vec3d((double) tempFacePos.getX() + 0.5, (double) tempFacePos.getY() + offsetForBlastResistant, (double) tempFacePos.getZ() + 0.5);
                    ...
                    if (!((double) damage < this.minimalDamage.get())) {
                        ...
                        if (!BlockUtils2.outOfPlaceRange(potentialPiston, Origin.VANILLA, this.mc.interactionManager.getReachDistance()) && (pistonState.isAir() || pistonState.getMaterial().isReplaceable() || this.skipPiston) && (!pistonState.isAir() || !EntityUtils.intersectsWithEntity(new Box(potentialPiston), Entity::isLiving))) {
                            Item redstone = null;
                            ...
                            label147:
                            {
                                ...
                                while (true) {
                                    ...
                                    if (!BlockUtils2.outOfPlaceRange(potentialRedstone, Origin.VANILLA, this.mc.interactionManager.getReachDistance())) {
                                        BlockState state = this.mc.world.getBlockState(potentialRedstone);
                                        if ((state.isAir() || state.getMaterial().isReplaceable() && state.emitsRedstonePower() && !pistonState.isAir() && this.mining && potentialRedstone.equals(this.torchPos)) && !EntityUtils.intersectsWithEntity(new Box(potentialRedstone), Entity::isLiving)) {
                                            if (potentialRedstone != places[0] || redstone == null || redstone != Items.REDSTONE_TORCH) {
                                                break;
                                            }

                                            FindItemResult supportBlock = InvUtils.findInHotbar(itemStack -> Block.getBlockFromItem(itemStack.getItem()).getBlastResistance() > 600.0F);
                                            if (supportBlock.found() && this.torchSupport.get()) {
                                                BlockPos downPos = potentialRedstone.down();
                                                if (!BlockUtils2.outOfPlaceRange(downPos, Origin.VANILLA, this.mc.interactionManager.getReachDistance())) {
                                                    BlockState supportState = this.mc.world.getBlockState(downPos);
                                                    if ((supportState.isAir() || supportState.isFullCube(this.mc.world, downPos) && !(supportState.getBlock().getBlastResistance() <= 600.0F)) && !EntityUtils.intersectsWithEntity(new Box(downPos), Entity::isLiving)) {
    ...
    private boolean canPlaceCrystal(BlockPos blockPos, boolean support) {
        ...
        if (!blockState.isOf(Blocks.BEDROCK) && !blockState.isOf(Blocks.OBSIDIAN) && !support) {
            ...
            return this.mc.world.getOtherEntities(null, new Box(x, y, z, x + 1, y + 2, z + 1)).isEmpty();
        }
    }

    private List<PlayerEntity> getTargets() {
        List<PlayerEntity> players = this.mc.world.getEntitiesByClass(PlayerEntity.class, new Box(this.mc.player.getBlockPos()).expand(this.targetRange.get().intValue()), Predicate.not(PlayerEntity::isMainPlayer));
        return players.isEmpty() ? players : players.stream().filter(LivingEntity::isAlive).filter(playerEntity -> Friends.get().shouldAttack(playerEntity)).sorted(Comparator.comparing(e -> this.mc.player.distanceTo(e))).collect(Collectors.toList());
    }
    ...
    private void mine(BlockPos blockPos, boolean override) {
        if (blockPos != null) {
            ...
            if (!this.mining && this.getBlock(blockPos).getHardness() >= 0.0F && !state.isAir() || override) {
                FindItemResult pickaxe = InvUtils.findInHotbar(Items.DIAMOND_PICKAXE, Items.NETHERITE_PICKAXE);
                ...
                this.mc.player.networkHandler.sendPacket(new PlayerActionC2SPacket(Action.START_DESTROY_BLOCK, blockPos, Direction.WEST));
                this.mc.player.swingHand(Hand.MAIN_HAND);
                this.mc.player.networkHandler.sendPacket(new PlayerActionC2SPacket(Action.STOP_DESTROY_BLOCK, blockPos, Direction.WEST));
                this.mining = true;
    ...
    private enum Stage {
        SEARCHING, CRYSTAL, REDSTONE, BREAKING
    }
}
