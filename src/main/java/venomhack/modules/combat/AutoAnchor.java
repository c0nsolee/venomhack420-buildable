...
public class AutoAnchor extends ModuleHelper implements IModule {
    ...
    public void doPlaceThreaded() {
        ...
        for (int x = -reach; x <= reach; ++x) {
            for (double y = Math.max(this.mc.world.getBottomY(), this.playerPos.y + this.eyeHeight.y - (double) reach); y <= Math.min(this.mc.world.getTopY(), this.playerPos.y + this.eyeHeight.y + (double) reach); ++y) {
        ...
        for (int x = -reach; x <= reach; ++x) {
            for (double y = Math.max(this.mc.world.getBottomY(), this.playerPos.y + this.eyeHeight.y - (double) reach); y <= Math.min(this.mc.world.getTopY(), this.playerPos.y + this.eyeHeight.y + (double) reach); ++y) {
                for (int z = -reach; z <= reach; ++z) {
                    Vec3d vec = new Vec3d((double) (this.mc.player.getBlockPos().getX() + x) + 0.5, y + 0.5, (double) (this.mc.player.getBlockPos().getZ() + z) + 0.5);
                    Mutable blockPos = new Mutable(vec.x, vec.y, vec.z);
                    if (!BlockUtils2.invalidPos(blockPos) && !(this.playerPos.add(this.eyeHeight).distanceTo(vec) > (UtilsPlus.cantSee(blockPos, this.strictDirections.get()) ? this.wallsRange.get() : this.range.get())) && this.isValidPlaceBlockState(this.mc.world.getBlockState(blockPos), blockPos) && this.isEmpty(blockPos)) {
                        boolean unsafeForFriends = false;
                        if (this.considerFriends.get()) {
                            for (LivingEntity friend : this.friends) {
                                Vec3d v = this.predict.get() ? UtilsPlus.smartPredictedPosition(friend, UtilsPlus.smartVelocity(friend)) : new Vec3d(0.0, 0.0, 0.0);
                        ...
                        if (!unsafeForFriends) {
                            for (LivingEntity target : this.targets) {
                                Vec3d v = this.predict.get() ? UtilsPlus.smartPredictedPosition(target, UtilsPlus.smartVelocity(target)) : new Vec3d(0.0, 0.0, 0.0);
