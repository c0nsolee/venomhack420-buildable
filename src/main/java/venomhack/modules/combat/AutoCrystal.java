...
public class AutoCrystal extends ModuleHelper implements IModule {
    ...
    private void onTick(Pre event) {
        ...
        if (PlayerUtils.getGameMode() != GameMode.SPECTATOR) {
            ...
            if (this.targets.isEmpty()) {
                ...
                if ((!this.pauseBedAura.get() || !Modules.get().isActive(AutoBed.class)) && (this.breakDelayLeft < 1 || this.delayBypass.get() != 0 && this.breaks < this.delayBypass.get()) && !this.pauseBreak()) {
                    this.doBreak();
                } else {
                    --this.breakDelayLeft;
                }
    ...
    private void onPacketRecieve(Receive event) {
        if (this.playerPos != null && this.mc.player != null && this.mc.world != null) {
            ...
            if (var6 instanceof EntitySpawnS2CPacket packet) {
                ...
                for (LivingEntity target : this.targets) {
                    if (!target.isDead() && crystal.distanceTo(target) <= 9.0F) {
                        float damage = this.applyHurtTimeToDamage(target, DamageCalcUtils.explosionDamage(target, crystal.getPos(), this.predict.get(), this.predictTicks.get(), this.antiStepOffset.get(), this.ignoreTerrain.get(), false, true, 6), false, false);
                        if ((double) damage >= (this.shouldFacePlace(target) ? 1.5 : this.minBreakDamage.get())) {
                            if (this.unsafeBreak(this.mc.player, crystal.getPos(), damage)) {
                                return;
                            }

                            if (this.antiFriendPop.get().breakTrue()) {
                                for (PlayerEntity friend : this.friends) {
                                    if (this.unsafeBreak(friend, crystal.getPos(), damage)) {
                                        return;
                                    }
                                }
                            }
                ...
                var6 = event.packet;
                if (var6 instanceof ExplosionS2CPacket packet) {
                    Vec3d pos = new Vec3d(packet.getX(), packet.getY(), packet.getZ());

                    for (Entity entity : this.mc.world.getEntities()) {
                        if (entity instanceof EndCrystalEntity && entity.squaredDistanceTo(pos) <= Math.pow(packet.getRadius() * 2.0F, 2.0)) {
                            this.crystalsToRemove.put(entity.getId(), System.currentTimeMillis());
                        }
                    }
                } else {
                    var6 = event.packet;
                    if (var6 instanceof EntitiesDestroyS2CPacket packet) {
                        IntListIterator var18 = packet.getEntityIds().iterator();
                        ...
                        var6 = event.packet;
                        if (var6 instanceof EntityTrackerUpdateS2CPacket packet) {
                            ...
                            Iterator var9 = packet.trackedValues.iterator();
                            if (healthDelta + absorbtionDelta > 0.0F) {
                                if (this.debug.get() && this.debugActualDamage.get()) {
                                    this.info("actual damage: " + TextUtils.round(healthDelta + absorbtionDelta, 4));
                                }

                                if ((long) living.timeUntilRegen - Math.round((double) this.getPing() / 50.0) > 10L) {
                                    this.lastTakenDamages.put(packet.id(), this.lastTakenDamages.getOrDefault(packet.id(), 0.0F) + healthDelta + absorbtionDelta);
                                } else {
                                        ...
                                        while (true) {
                                            while (true) {
                                                if (!var20.hasNext()) {
                                                    ...
                                                    if ((bestPlace.getBlockPos() == null || bestPlace.getSurroundBreak() != 0) && !Modules.get().isActive(AutoFunnyCrystal.class) && UtilsPlus.isSurrounded(target, true, this.ignoreTerrain.get())) {
    ...
    private BlockState getState(BlockPos pos) {
        return Modules.get().get(PacketMine.class).getState(pos);
    }
    ...
    private void doAttack(EndCrystalEntity crystal, boolean predict, @Nullable PlacementInfo surroundReplace) {
        boolean canSilentWeaknessSwitch = this.antiWeaknessSlot != -1 && this.canBreakCrystal(this.mc.player.getInventory().getStack(this.antiWeaknessSlot), 0);
        ...
        if (cantBreakRn) {
            ...
            if (!canSilentWeaknessSwitch || this.switchMode.get() != SwitchMode.SILENT || this.afterSwitchBreakPause.get() > 0 && this.silentMode.get() == SwapMode.OLD) {
        ...
        if (this.rotationMode.get().breakTrue()) {
            if (this.hasRotatedThisTick || this.yawstep.get() != 180 && !this.validRotation(crystal.getPos())) {
                ...
                Rotations.rotate(Rotations.getYaw(crystal), Rotations.getPitch(crystal, this.breakRotationMode.get()), 85, () -> this.attackCrystal(crystal, predict, cantBreakRn, surroundReplace));
                this.hasRotatedThisTick = true;
            }
        } else {
    ...
    private void attackCrystal(EndCrystalEntity crystal, boolean predict, boolean shouldAntiWeakness, @Nullable PlacementInfo surroundReplace) {
        ...
        if (surroundReplace != null && this.placeDelayLeft < 1) {
            this.place(surroundReplace, this.rotationMode.get().placeTrue());
        }
        ...
        if (this.debug.get()) {
            if (this.debugActualDamage.get() && tries == 0) {
                float selfDamage = this.applyHurtTimeToDamage(this.mc.player, DamageCalcUtils.explosionDamage(this.mc.player, crystal.getPos(), 6), false, false);
                if (TextUtils.round(selfDamage, 4) > 0.0) {
                    this.info("calculated damage: " + TextUtils.round(selfDamage, 4));
                }
            }

            if (this.debugAttackedId.get()) {
                this.info("id: " + crystal.getId());
            }
        }
    }
    ...
    private boolean pauseBreak() {
        if (this.breaks >= this.maxBreaksPerSecond.get()) {
            return true;
        } else if (!this.switchTimer.passedMillis(this.afterSwitchBreakPause.get().intValue())) {
            return true;
        } else if (this.pauseMode.get().breakTrue() && PlayerUtils.shouldPause(this.pauseOnMine.get(), this.pauseOnEat.get(), this.pauseOnEat.get())) {
            return true;
        } else {
            this.weak = false;
            if (this.antiWeakness.get()) {
                Map<StatusEffect, StatusEffectInstance> effects = this.mc.player.getActiveStatusEffects();
                StatusEffectInstance weakness = effects.get(StatusEffects.WEAKNESS);
                StatusEffectInstance strength = effects.get(StatusEffects.STRENGTH);
    ...
    private Hand getHand() {
        return this.mc.player.getMainHandStack().getItem() != Items.END_CRYSTAL && this.mc.player.getOffHandStack().getItem() == Items.END_CRYSTAL ? Hand.OFF_HAND : Hand.MAIN_HAND;
    }
    ...
    private boolean canBreakCrystal(ItemStack stack, int timeSinceLastCooldownReset) {
        ...
        for (StatusEffectInstance instance : this.mc.player.getStatusEffects()) {
            if (instance.getEffectType() == StatusEffects.STRENGTH) {
                damage += 3 * (instance.getAmplifier() + 1);
            } else if (instance.getEffectType() == StatusEffects.WEAKNESS) {
                damage -= 4 * (instance.getAmplifier() + 1);
    ...
    public String getInfoString() {
        return this.targets.isEmpty() ? null : this.targets.get(0).getEntityName();
    }
