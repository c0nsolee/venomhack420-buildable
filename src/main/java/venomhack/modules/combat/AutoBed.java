...
public class AutoBed extends ModuleHelper implements IModule {
    ...
    private final Setting<venomhack.enums.Type> antiFriendPop = this.setting("anti-friend-pop", "Takes into account damage to friends.", Type.BOTH, this.sgTargeting);
    ...
    private void onPreTick(Pre event) {
        if (this.mc.world.getDimension().bedWorks()) {
            this.error("You are in the Overworld... disabling!");
            this.toggle();
        } else {
            ...
            PlayerUtils2.collectTargets(this.targets, this.friends, this.targetRange.get(), this.maxTargets.get(), false, this.onlyHoled.get(), this.ignoreTerrain.get(), this.sortPrio.get());
            if (this.targets.isEmpty()) {
                this.target = null;
            }

            if (this.placeDelayLeft <= 0) {
                if (this.place.get() && InvUtils.find(itemStack -> itemStack.getItem() instanceof BedItem).count() >= Math.max(1, this.minBedAmount.get()) && !this.shouldPause() && !this.targets.isEmpty()) {
            ...
            if (this.craftDelayLeft > 0) {
                ...
                if (!this.cantCraft()) {
                    if (this.debugText.get()) {
                        this.info("Looking for crafting table.");
                    }
                        ...
                        for (int y = -reach; y <= reach; ++y) {
                            for (int z = -reach; z <= reach; ++z) {
                                ...
                                if (this.inRange(blockPos) && !BlockUtils2.invalidPos(blockPos)) {
                                    ...
                                    if (state.getMaterial().isReplaceable() && (!this.noAirplace.get() && !this.oldMode.get() || !BlockUtils2.noSupport(blockPos)) && (potential == null || potential.getSquaredDistance(this.mc.player.getBlockPos()) < blockPos.getSquaredDistance(this.mc.player.getBlockPos()))) {
                                        potential = blockPos;
                                    }
                                }
                            }
                    ...
                    if (this.placeTables.get() && potential != null) {
                        FindItemResult tables = InvUtils.findInHotbar(Items.CRAFTING_TABLE);
                        if (!tables.found()) {
                            return;
                        }

                        if (this.debugText.get()) {
                            this.info("Placing crafting table");
                        }

                        BlockUtils2.placeBlock(tables, potential, this.rotateChecks.get(), 20, !this.noAirplace.get() && !this.oldMode.get(), false, this.swing.get(), this.strictDirections.get());
    ...
    private void onPostTick(Post event) {
        ...
        if (this.breakDelayLeft <= 0) {
            if (!this.pauseAll() && !this.targets.isEmpty()) {
                if (this.dumbBreak == null) {
                    ...
                    if (this.debugText.get()) {
                        this.info("fast break: " + this.dumbBreak.placeDirection);
                    }

                    if (this.rotateChecks.get()) {
                        Rotations.rotate(Rotations.getYaw(this.dumbBreak.pos), Rotations.getPitch(this.dumbBreak.pos), 50, false, () -> this.breakBed(this.dumbBreak, true));
                    } else {
        ...
        BedRenderBlock.bedTick(this.renderBeds);
        ...
        if (this.lastMineTarget != null && this.lastMineTarget.pos != null) {
            if (!this.inRange(this.lastMineTarget.pos) || !this.isGoodTarget(this.lastMineTarget.target) || this.breakMode.get() != Mode.INSTANT && this.mc.world.getBlockState(this.lastMineTarget.pos).isAir()) {
                if (this.lastMineTarget.placeDirection != null && this.placeDelayLeft <= 0) {
                    this.dumbPlace = this.lastMineTarget;
                }

                if (this.prevSlot != -1 && PlayerUtils.getTotalHealth() >= (double) this.minMineHealth.get().intValue() && !this.pauseAll()) {
                    InvUtils.swap(this.prevSlot, false);
                }

                if (this.debugText.get()) {
                    this.info("Resetting lastMinePos");
                }

                this.lastMineTarget = null;
                this.mineTimeLeft = -1;
                this.prevSlot = -1;
            } else if (!this.pauseAll() && PlayerUtils.getTotalHealth() >= (double) this.minMineHealth.get().intValue()) {
                this.doBreakSwitch(this.lastMineTarget.pos);
            }
        }

        if (PlayerUtils.getTotalHealth() >= (double) this.minMineHealth.get().intValue() && !this.shouldPause() && (!this.mineSafe.get() || UtilsPlus.isSafe(this.mc.player))) {
            if (this.lastMineTarget != null && (this.mineTimeLeft >= 0 || this.breakMode.get() == Mode.INSTANT) && this.mineTimeLeft >= -10) {
                if (this.lastMineTarget.pos != null && PlayerUtils.getTotalHealth() >= (double) this.minMineHealth.get().intValue() && !this.pauseAll()) {
                    ...
                    if (!state.getMaterial().isReplaceable() && !(state.getBlock() instanceof BedBlock) && !state.isOf(Blocks.BEDROCK)) {
                        --this.mineTimeLeft;
                        float[] rotation = PlayerUtils.calculateAngle(new Vec3d((double) this.lastMineTarget.pos.getX() + 0.5, (double) this.lastMineTarget.pos.getY() + 0.5, (double) this.lastMineTarget.pos.getZ() + 0.5));
                        switch (this.breakMode.get()) {
                            case BYPASS:
                                if (this.rotateChecks.get()) {
                                    Rotations.rotate(rotation[0], rotation[1], 25, () -> this.mc.interactionManager.updateBlockBreakingProgress(this.lastMineTarget.pos, BlockUtils2.getClosestDirection(this.lastMineTarget.pos, false)));
                                } else {
                            ...
                            case INSTANT:
                                if (this.rotateChecks.get()) {
                                    Rotations.rotate(rotation[0], rotation[1], 25, () -> this.mc.getNetworkHandler().sendPacket(new PlayerActionC2SPacket(Action.STOP_DESTROY_BLOCK, this.lastMineTarget.pos, BlockUtils2.getClosestDirection(this.lastMineTarget.pos, false))));
                                } else {
                                    this.mc.getNetworkHandler().sendPacket(new PlayerActionC2SPacket(Action.STOP_DESTROY_BLOCK, this.lastMineTarget.pos, BlockUtils2.getClosestDirection(this.lastMineTarget.pos, false)));
                                }
            ...
            } else if (InvUtils.findInHotbar(itemStack -> itemStack.getItem().equals(Items.DIAMOND_PICKAXE) || itemStack.getItem().equals(Items.NETHERITE_PICKAXE)).found()) {
                for (PlayerEntity target : this.targets) {
                    ...
                    if (this.antiAntiBed.get() && !UtilsPlus.isTrapped(target) && !this.xymb.get()) {
                        ...
                        if (this.inRange(target.getBlockPos().up()) && !state.getMaterial().isReplaceable() && !(state.getBlock() instanceof BedBlock) && !state.isOf(Blocks.BEDROCK)) {
                            this.mineTimeLeft = CityUtils.getSpeed(state, target.getBlockPos().up());
                            this.lastMineTarget = new BedData(target.getBlockPos().up(), null, 10.0F, target);
                            this.doBreakSwitch(this.lastMineTarget.pos);
                            this.mineBlock(target.getBlockPos().up());
                            RenderBlock.addRenderBlock(this.mineBlocks, target.getBlockPos().up(), this.mineTimeLeft + 1);
                            if (this.debugText.get()) {
                                this.info("Obstruction mining " + target.getEntityName());
                            }
                            break;
                        }
                    }
                    ...
                    if (this.selfTrapMine.get() && !this.xymb.get() && (UtilsPlus.isTrapped(target) || this.breakMode.get() == Mode.PACKET && this.mc.world.getBlockState(target.getBlockPos().up()).getBlock() instanceof BedBlock)) {
                        for (Vec3i city : CityUtils.CITY_WITHOUT_BURROW) {
                            BlockState state = this.mc.world.getBlockState(target.getBlockPos().add(city).up());
                            if ((!surround.isActive() || !surround.getPlacePositions(false).contains(new BlockPos(city))) && (!selftrap.isActive() || !surround.getPlacePositions(false).contains(new BlockPos(city))) && !state.getMaterial().isReplaceable() && !(state.getBlock() instanceof BedBlock) && !state.isOf(Blocks.BEDROCK) && this.inRange(target.getBlockPos().add(city).up())) {
                                this.mineTimeLeft = CityUtils.getSpeed(state, target.getBlockPos().add(city).up());
                                this.lastMineTarget = new BedData(target.getBlockPos().add(city).up(), RandUtils.direction(city).getOpposite(), this.getBedDamage(Vec3d.ofCenter(target.getBlockPos().up())), target);
                                this.doBreakSwitch(this.lastMineTarget.pos);
                                ...
                                if (this.debugText.get()) {
                                    this.info("Self trap mining " + target.getEntityName());
                                }
                        ...
                        if (this.mineHead.get() && (!surround.isActive() || !surround.getPlacePositions(false).contains(target.getBlockPos().up(2))) && (!selftrap.isActive() || !surround.getPlacePositions(false).contains(target.getBlockPos().up(2)))) {
                            BlockState state = this.mc.world.getBlockState(target.getBlockPos().up(2));
                            if (!state.getMaterial().isReplaceable() && !(state.getBlock() instanceof BedBlock) && !state.isOf(Blocks.BEDROCK) && this.inRange(target.getBlockPos().up(2))) {
                                this.mineTimeLeft = CityUtils.getSpeed(state, target.getBlockPos().up(2));
                                this.lastMineTarget = new BedData(target.getBlockPos().up(2), null, 10.0F, target);
                                this.doBreakSwitch(this.lastMineTarget.pos);
                                ...
                                if (this.debugText.get()) {
                                    this.info("Self trap head mining " + target.getEntityName());
                                }
                                break;
                            }
                        }
    ...
    private void doPlaceThreaded() {
        if (this.placeDelayLeft <= 0) {
            ...
            for (int x = -range; x <= range; ++x) {
                for (double y = -range; y <= (double) range; ++y) {
                    label171:
                    for (int z = -range; z <= range; ++z) {
                        BlockPos blockPos = new Mutable(origin.x + (double) x, origin.y + y, origin.z + (double) z);
                        if (!BlockUtils2.invalidPos(blockPos)) {
                            Vec3d currentPosVec = Vec3d.ofCenter(blockPos);
                            if ((!this.xymb.get() || this.mc.world.canPlace(Blocks.PURPLE_BED.getDefaultState(), blockPos, ShapeContext.absent())) && (!this.oldMode.get() || this.mc.world.getBlockState(blockPos.down()).getMaterial().isSolid())) {
                                ...
                                if (placeDirection != null) {
                                    ...
                                    double selfDmg = DamageCalcUtils.explosionDamage(this.mc.player, currentPosVec, false, this.ignoreTerrain.get(), true, 5);
                                    if (!(selfDmg > this.maxPlaceSelfDamage.get()) && (!this.noSuicidePlace.get() || !(PlayerUtils.getTotalHealth() - selfDmg < 0.5))) {
                                        if (this.antiFriendPop.get().placeTrue()) {
                                            for (PlayerEntity friend : this.friends) {
                                                if (!((double) DamageCalcUtils.explosionDamage(friend, currentPosVec, this.predict.get(), false, false, 5) < this.maxPlaceSelfDamage.get())) {
                                                    double friendDmg = DamageCalcUtils.explosionDamage(friend, currentPosVec, this.predict.get(), this.ignoreTerrain.get(), true, 5);
                                                    if (friendDmg > this.maxPlaceSelfDamage.get() || this.noSuicidePlace.get() && (double) EntityUtils.getTotalHealth(friend) - friendDmg < 0.5) {
                                                        continue label171;
                                                    }
                                                }
                                            }
                                        ...
                                        for (PlayerEntity target : this.targets) {
                                            if (!this.smartDelay.get() || target.hurtTime <= this.hurtTimeThreshold.get()) {
                                                ...
                                                float damage = DamageCalcUtils.explosionDamage(target, currentPosVec, this.predict.get(), this.ignoreTerrain.get(), true, 5);
                                                if (this.netDamage.get()) {
                                                    totalDamage += damage;
                                                ...
                                                if (!((double) damage < this.minPlaceDamage.get())) {
                                                    ...
                                                    if (this.trapHoldTarget == null || !UtilsPlus.isSurrounded(target, true, true) || target.equals(this.trapHoldTarget)) {
                                                        ...
                                                        if (!this.netDamage.get()) {
                                                            bestBed.set(new Mutable(offsetCurrentPosVec.x, offsetCurrentPosVec.y, offsetCurrentPosVec.z), placeDirection, damage, target);
                                                        }
                                                    }
                                                }
                                            }
                                        ...
                                        if (this.netDamage.get() && enoughSingleDamage && getPooped.size() <= poopsHere.size() && !(bestBed.damage > totalDamage)) {
                                            ...
                                            bestBed.set(new Mutable(offsetCurrentPosVec.x, offsetCurrentPosVec.y, offsetCurrentPosVec.z), placeDirection, totalDamage, this.trapHoldTarget == null ? this.targets.get(0) : this.trapHoldTarget);
                                        }
                                    }
                                }
                            }
    ...
    private boolean cantCraft() {
        if (!this.autoCraft.get()) {
            ...
            return true;
        } else if (PlayerUtils.getTotalHealth() < (double) this.minCraftHealth.get().intValue()) {
            return true;
            ...
            AutoCrafter crafter = Modules.get().get(AutoCrafter.class);
            ...
            if (!beds.found() || beds.count() <= this.refillThreshold.get() && beds.count() < crafter.maxBeds.get()) {
                ...
                if (emptySlots < this.minBedCraftAmount.get() || emptySlots == 0) {
                    ...
                    if (!this.hasWarnedNoCrafter) {
                        this.error("Enable Auto Crafter!");
                    ...
                    if (crafter.noBedMaterials()) {
                        if (!this.hasWarnedNoMats) {
                            this.error("Not enough materials!");
    ...
    private Direction getPlaceDirection(BlockPos blockPos) {
        ...
        Vec3d origin = this.placeOrigin.get().getOrigin(this.playerPos);
        Iterator var6 = Arrays.stream((Object[]) (new Direction[]{Direction.NORTH, Direction.EAST, Direction.SOUTH, Direction.WEST})).iterator();

        label61:
        while (var6.hasNext()) {
            Direction direction = (Direction) var6.next();
            BlockPos offsetBlockPos = blockPos.offset(direction);
            Vec3d offsetPosVec = Vec3d.ofCenter(offsetBlockPos);
            if (this.rotateChecks.get()) {
                boolean foundOne = false;
                Vec3d hitVec = offsetPosVec;
                Iterator var12 = Arrays.asList(Direction.NORTH, Direction.SOUTH).iterator();

                label44:
                while (true) {
                    if (var12.hasNext()) {
                        Direction direction1 = (Direction) var12.next();
                        hitVec = hitVec.add(Vec3d.ofCenter(direction1.getVector()).multiply(0.5));
                        Iterator var14 = Arrays.asList(Direction.WEST, Direction.EAST).iterator();
                        ...
                        do {
                            ...
                            offsetYaw = Rotations.getYaw(hitVec);
                        } while (!Direction.fromRotation(offsetYaw).equals(direction.getOpposite()));
            ...
            if (this.inRange(offsetPosVec)) {
                ...
                if (!(distanceCurrent > distance) && this.mc.world.canPlace(Blocks.PURPLE_BED.getDefaultState(), offsetBlockPos, ShapeContext.absent())) {
                    BlockState offState = this.mc.world.getBlockState(offsetBlockPos);
                    if (offState.getMaterial().isReplaceable() && (!this.oldMode.get() || offState.isAir() && this.mc.world.getBlockState(offsetBlockPos.down()).isSolidBlock(this.mc.world, blockPos)) && (!this.noAirplace.get() || !BlockUtils2.noSupport(offsetBlockPos))) {
