...
public class AutoEz extends ModuleHelper {
    ...
    private void onDeath(PlayerDeathEvent event) {
        ...
        if (this.delayLeft <= 0 && event.isTarget() && (!this.chat.get() || event.isChat())) {
            if (this.messages.get().isEmpty()) {
                this.warning("You have no messages set for Auto eZ.");
            } else {
                this.delayLeft = this.delay.get();
                StringBuilder msg = new StringBuilder();
                msg.append(TextUtils.getNewMessage(this.messages.get()));
                if (this.sayKillStreak.get() && Math.max(1, Venomhack420.STATS.getKillStreak()) % this.ksCount.get() == 0) {
                    msg.delete(0, msg.length());
                    msg.append(this.ksMsg.get());
                } else if (this.addKsSuffix.get()) {
                    msg.append(this.ksSuffixMsg.get());
                }
                ...
                TextUtils.sendNewMessage(msg.toString().replace("{player}", victim.getGameProfile().getName()).replace("{pops}", Venomhack420.STATS.getTotemPops(victim) + "").replace("{totem}", Venomhack420.STATS.getTotemPops(victim) == 1 ? "totem" : "totems"));
            }
        }
    }
