...
public class Notifier extends ModuleHelper {
            ...
            if (breakingPlayerx != null && !breakingPlayerx.equals(this.mc.player) && packet.getProgress() <= 0 && !breakingPlayerx.equals(this.prevBreakingPlayer)) {
                ...
                for (Direction dir : Direction.values()) {
                    if (packet.getPos().equals(playerBlockPos.offset(dir))) {
                        ChatUtils.warning("Your " + dir.getName() + " surround block is being broken by " + breakingPlayerx.getEntityName());
                ...
                if (packet.getPos().equals(this.mc.player.getBlockPos())) {
                    ChatUtils.warning("Your burrow is being mined by " + breakingPlayerx.getEntityName());
                }
            }
        }
    }
                        ...
                        if (!this.burrowedPlayers.contains(player)) {
                            ...
                            if (!state.getOutlineShape(this.mc.world, player.getBlockPos()).isEmpty()) {
                                if (!this.renderBurrow.get()) {
                                    this.info(player.getEntityName() + " just burrowed in " + state.getBlock().getName().getString() + "!");
                                }
                ...
                if (this.effectNotifier.get() && (this.friendsEffects.get() || Friends.get().shouldAttack(player))) {
                    ...
                    if (this.egap.get() && playerEffects.containsKey(StatusEffects.ABSORPTION)) {
                        ...
                        if (effect.getDuration() == 2400) {
                            ...
                            this.info(player.getEntityName() + " just ate an Egap! " + this.egapPlayers.get(id) + " in total.");
                        }
                    }
    ...
    private void notifyEffect(PlayerEntity player, StatusEffectInstance effect) {
        ...
        this.info(player.getEntityName() + " now has " + Text.translatable(effect.getTranslationKey()) + " " + (effect.getAmplifier() + 1) + " for " + (minutes >= 1 ? minutes + ":" + (modulo < 10 ? "0" : "") + modulo + " minutes!" : seconds + " seconds!"));
    }
