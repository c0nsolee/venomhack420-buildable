...
public class ArmorMessage extends ModuleHelper {
    ...
    private void onTick(Post event) {
        for (Entity entity : this.mc.world.getEntities()) {
            if (entity instanceof PlayerEntity player && (player.equals(this.mc.player) || this.friends.get() && !Friends.get().shouldAttack(player)) && PlayerUtils2.getGameMode(player) != GameMode.CREATIVE && PlayerUtils2.getGameMode(player) != GameMode.SPECTATOR) {
                for (ItemStack itemStack : player.getArmorItems()) {
                    if (itemStack != null && !itemStack.isEmpty() && itemStack.isDamageable()) {
                        ...
                        if (!(damage > this.threshold.get() | this.armor.put(index, damage) <= this.threshold.get())) {
                            String msg = this.message.get().replace("{piece}", piece).replace("{grammar}", grammar).replace("{percent}", damage + "").replace("{player}", player.getEntityName());
                            if (player == this.mc.player) {
                                if (this.toastNotification.get()) {
                                    this.mc.getToastManager().add(new ArmorNotificationToast(itemStack.getItem(), "Your " + piece + " " + grammar + " on " + damage + "%"));
                                } else {
    ...
    public WWidget getWidget(GuiTheme theme) {
        ...
        placeholders.action = () -> new GuideScreen().withContents("These are the placeholders for armor message.", "They can't be modified and only work for this module.", "", "{piece} gets replaced for the armor piece that dropped below the durability threshold.", "{grammar} returns 'is' or 'are', depending on the type of armor.", "{percent} just returns the percentage value of the durability of the armor piece.", "{player} will return the player whos armor is running low.", "").show();
        return list;
    }
    ...
    private class ArmorNotificationToast extends MeteorToast {
        public ArmorNotificationToast(@Nullable Item item, @Nullable String text) {
            super(item, ArmorMessage.this.title(), text, ArmorMessage.this.duration.get() * 1000);
        }

        public SoundInstance getSound() {
            return PositionedSoundInstance.master(ArmorMessage.this.sound.get().isEmpty() ? SoundEvents.ENTITY_ITEM_BREAK : (SoundEvent) ((List) ArmorMessage.this.sound.get()).get(0), 1.2F, 1.0F);
        }
    }
}
