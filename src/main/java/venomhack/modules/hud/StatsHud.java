...
public class StatsHud extends HudElement {
    ...
    private final Setting<Boolean> kills = this.sgGeneral.add(new Builder().name("kill-counter").description("Displays how many kills you got in this session.").defaultValue(true).build());
    private final Setting<Boolean> killstreak = this.sgGeneral.add(new Builder().name("killstreak").description("Displays how many kills you got in a row.").defaultValue(true).build());
    private final Setting<Boolean> kayDee = this.sgGeneral.add(new Builder().name("k/d").description("Displays your current kills / deaths ratio.").defaultValue(true).build());
    private final Setting<Boolean> deaths = this.sgGeneral.add(new Builder().name("death-counter").description("Displays how many times you died in this session.").defaultValue(true).build());
    public StatsHud() {
        super(INFO);
    }    public static final HudElementInfo<StatsHud> INFO = new HudElementInfo(Venomhack420.HUD_GROUP, "statistics", "Displays certain pvp related statistics.", StatsHud::new);
    ...
    public void render(HudRenderer renderer) {
        ...
        if (MeteorClient.mc.world != null && (this.kills.get() || this.killstreak.get() || this.kayDee.get() || this.deaths.get())) {
            ...
            if (this.killstreak.get()) {
                ...
                renderer.text(" " + Math.round(Venomhack420.STATS.getKillStreak()), x + renderer.textWidth("Killstreak:"), y, Color.WHITE, true);
                y += renderer.textHeight() + 2.0;
            }
