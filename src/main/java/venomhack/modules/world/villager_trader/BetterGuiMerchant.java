...
public class BetterGuiMerchant extends MerchantScreen {
    ...
    public void trade(int tradeIndex) {
        ...
        while (!recipe.isDisabled() && this.client.player.currentScreenHandler.getCursorStack().isEmpty() && this.inputSlotsAreEmpty() && this.hasWantedTradeItems() && this.hasEnoughItemsInInventory(recipe) && this.canReceiveOutput(recipe.getSellItem())) {
    ...
    private boolean inputSlotsAreEmpty() {
        return this.handler.getSlot(0).getStack().isEmpty() && this.handler.getSlot(1).getStack().isEmpty() && this.handler.getSlot(2).getStack().isEmpty();
    }

    private boolean hasEnoughItemsInInventory(TradeOffer recipe) {
        return this.hasEnoughItemsInInventory(recipe.getAdjustedFirstBuyItem()) && this.hasEnoughItemsInInventory(recipe.getSecondBuyItem());
    }
    ...
    private boolean hasEnoughItemsInInventory(ItemStack stack) {
        ...
        for (int i = this.handler.slots.size() - 36; i < this.handler.slots.size(); ++i) {
            ItemStack invstack = this.handler.getSlot(i).getStack();
            if (invstack != null) {
