        ...
        if (this.bambooResult.slot() != -1) {
            ...
            if (this.tickdelay <= 0) {
                ...
                BlockIterator.register((int) Math.ceil(this.range.get()), (int) Math.ceil(this.range.get()), (blockPos, blockState) -> {
                    ...
                    if (!(this.headCenterPos.distanceTo(vecpos) > this.range.get()) && (blockState.getBlock() instanceof SandBlock || blockState.isOf(Blocks.GRAVEL) || blockState.isOf(Blocks.DIRT) || blockState.isOf(Blocks.COARSE_DIRT) || blockState.isOf(Blocks.GRASS_BLOCK))) {
