...
public class ObsidianFarm extends ModuleHelper {
    ...
    private final Setting<When> when = this.setting("when", "When to activate.", When.DYNAMIC);
    private boolean t;
    ...
    private void onTick(Pre event) {
            ...
            case DYNAMIC:
                if (this.count > 0 && this.mc.player != null) {
                    if (PlayerUtils.getDimension() != this.dim) {
                        TextUtils.sendNewMessage(this.message.get());
                    }
        ...
        if (PlayerUtils.getDimension() == this.switchDim && this.t) {
            TextUtils.sendNewMessage(this.message.get());
            this.t = false;
        } else if (PlayerUtils.getDimension() != this.switchDim) {
            this.t = true;
        }
    }

    public enum When {
        OVERWORLD("Overworld"), NETHER("Nether"), END("End"), DYNAMIC("Dynamic");
