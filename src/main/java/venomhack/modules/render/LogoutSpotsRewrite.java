...
public class LogoutSpotsRewrite extends ModuleHelper {
    ...
    private void onRender2D(Render2DEvent event) {
        this.spots.forEach(spot -> {
            ...
            if (spot.dimension == PlayerUtils.getDimension() && RandUtils.horizontalDistance(spot.player.getPos(), this.mc.player.getPos()) < (double) this.mc.gameRenderer.getViewDistance() && NametagUtils.to2D(vec, this.scale.get())) {
                PlayerEntity player = spot.player;
                ...
                renderer.beginBig();
                renderer.render(player.getEntityName(), totalWidth * -0.5, -height, PlayerUtils.getPlayerColor(player, this.nameColor.get()), true);
