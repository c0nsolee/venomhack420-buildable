...
public class ElytraFlightModeMixin {
    ...
    public void handleHorizontalSpeed(PlayerMoveEvent event) {
        ...
        double hSpeed = PlayerUtils.getDimension() == Dimension.Nether && MeteorClient.mc.player.getY() > 128.0 ? ((IElytraFly) this.elytraFly).getRoofSpeed() : this.elytraFly.horizontalSpeed.get();
        hSpeed *= tickFactor;
        ...
        if (MeteorClient.mc.options.jumpKey.isPressed() && !MeteorClient.mc.options.sneakKey.isPressed()) {
            vSpeed = PlayerUtils.getDimension() == Dimension.Nether && MeteorClient.mc.player.getY() > 128.0 ? ((IElytraFly) this.elytraFly).getRoofSpeed() : this.elytraFly.verticalSpeed.get();
