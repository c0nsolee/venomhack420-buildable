    ...
    public static boolean isFucked(LivingEntity target) {
        ...
        if (isBurrowed(target)) {
            ...
            if (count == 4) {
                ...
                return true;
            } else if (!MeteorClient.mc.world.getBlockState(target.getBlockPos().add(0, 2, 0)).getOutlineShape(MeteorClient.mc.world, target.getBlockPos().add(0, 2, 0)).isEmpty()) {
                return true;
            } else {
    ...
    public static Vec3d smartPredictedPosition(Entity entity, Vec3d movement) {
        ...
        if (entity.getStepHeight() > 0.0F && bl4 && (bl || bl3)) {
            ...
            if (vec3d2.horizontalLengthSquared() > vec3d.horizontalLengthSquared()) {
                return vec3d2.add(adjustMovementForCollisions(entity, new Vec3d(0.0, -vec3d2.y + movement.y, 0.0), box.offset(vec3d2), MeteorClient.mc.world, list));
            }
        }
